...
- Filter by WBS requirement (apply only to flats that require a WBS, or only to those that don't)
- Apply automatically via application form and log applications
- **Telegram notifications**: receive an instant message whenever an application is submitted, including flat title, location, size, rooms, rent, WBS status, and a link to the listing
- **Persistent application log**: `flats.log` is stored on a separate `log` branch and updated after every run, so the history of applications is preserved across GitHub Actions runs without cluttering the main branch
...
To fork the repository, click on the _Fork_ button at the top right of the repository main page:
...
**3. Remove comments in front of "schedule" and "- cron: '_/5 6-19 _ \* 1-5'" in the file `.github/workflows/run-scraper.yml`**
